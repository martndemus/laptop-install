...
./install.sh --skip=macos-settings        # run all except these
./install.sh --help
```
...
| Step | Description |
| --- | --- |
| `preflight` | Check Full Disk Access is granted and install Xcode Command Line Tools if missing |
...
Steps are auto-discovered from the `steps/` directory and ordered by their numeric prefix. To add a new step, drop in a new file like `steps/06-foo.sh` — no edit to `install.sh` required.
...
- The `preflight` step may exit early and ask you to complete a GUI prompt the first time Command Line Tools are installed; re-run after.
